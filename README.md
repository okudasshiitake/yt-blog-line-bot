...
| `PLATFORM` | `BASE` | 投稿先（`BASE` または `note`）。noteにするとMarkdown・まとめセクション等が自動適用 |
| `CUSTOM_RULES` | `関西弁で書くこと。語尾は「やねん」などを使用する。` | オプション：自分だけの執筆ルールを追加したい場合に入力 |

※ 使わない項目（例：`NOT_SELLING`、`CUSTOM_RULES`、`PLATFORM`）は未入力のままで動きます（デフォルトはBASEモード）。
